...
public class SellerController {
    ...
    /**
     * GET /api/sellers/{sellerId}/transfer-history
     * Lấy lịch sử chuyển tiền của seller.
     */
    public void getTransferHistory(Context ctx) {
        try {
            long sellerId = Long.parseLong(ctx.pathParam("sellerId"));
            Response response = sellerService.getTransferHistory(sellerId); // Gọi service mới
            ctx.json(response);
        } catch (NumberFormatException e) {
            ctx.status(400).json(new Response("ERROR", "Seller ID không hợp lệ", null));
        } catch (Exception e) {
            logger.error("Lỗi lấy lịch sử chuyển tiền của seller: {}", e.getMessage(), e);
            ctx.status(500).json(new Response("ERROR", "Lỗi server: " + e.getMessage(), null));
        }
    }
}
